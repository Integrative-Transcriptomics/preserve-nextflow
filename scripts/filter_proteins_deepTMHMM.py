...
for line in protein_top_list:
    if (include_filter in line) and (exclude_filter not in line) and ("_" in line):
...
for i in protein_subset:
    ...
    for j in protein_seq_list:
        if i[1:13] in j:
            ...
            seq_index_start = id_index + 1
            return_list.append(protein_seq_list[id_index])
            return_list.append(protein_seq_list[seq_index_start])

# write the protein subset to a new file
with open("filtered_cluster_reps.fasta_deepTMHMM", "w") as outfile:
